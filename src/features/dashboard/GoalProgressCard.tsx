...
export function GoalProgressCard({ className }: GoalProgressCardProps) {
  ...
  return (
    <DashboardCard
      className={cn(
        "h-full min-h-0 min-w-0 max-w-full bg-gradient-to-br from-bloomora-white from-0% via-bloomora-lilac/[0.045] via-[48%] to-bloomora-mist to-100% p-5 sm:p-6",
        className,
      )}
    >
      <header className="flex min-w-0 shrink-0 flex-wrap items-start justify-between gap-3 border-b border-bloomora-line/15 pb-4">
        ...
        <div className="flex min-w-0 shrink-0 flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-end sm:gap-3">
          <Link
            to="/app/goals/new"
            className={buttonClassName({
              ...
              size: "sm",
              className:
                "shrink-0 font-semibold text-bloomora-violet hover:text-bloomora-deep",
            })}
          >
            Nueva meta
          </Link>
      ...
      <div className="mt-5 flex min-h-0 w-full max-w-full flex-1 flex-col">
        <div
          className={cn(
            "agenda-task-scroll relative z-[1] min-h-0 w-full max-w-full flex-1 rounded-xl ring-1 ring-bloomora-line/25",
          )}
        >
          <div className="flex flex-col gap-6 px-0.5 pb-1 pt-0.5">
            <div className="flex w-full min-w-0 max-w-full flex-col items-center overflow-x-clip text-center">
              {isLoading ? (
